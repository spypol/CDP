<?php
	...
	if($isLien == 0){
		...
		$target_path = $_SERVER['DOCUMENT_ROOT'] . "/admin/culture/";;
	...
	if(isset($_POST['nomFichier'])) {
		...
		if($isLien == 0){
			$lien = 'http://www.souvienstoipan.com/admin/culture/' . $nomFichierLien;
			$sql = 'INSERT INTO T_CULTURE (culture_NOM, culture_FICHIER, culture_VALIDE, culture_CAT) VALUES ("'.$nom.'", "'.$lien.'", 1, 0)';
		}else{
			$lien = $nomFichierLien;
			$sql = 'INSERT INTO T_CULTURE (culture_NOM, culture_LIEN, culture_VALIDE, culture_CAT) VALUES ("'.$nom.'", "'.$lien.'", 1, 1)';
		}
		...
		$message = "<html>
		<head>
		  <title>[Souviens-toi Pan !] Nouveau document de culture uploadé sur l'intranet</title>
		</head>
		<body>
		  <table>
		  	<tr><td></td></tr>
		    <tr>
		      <td style='font-size:12px;font-weight:bold;font-family: Arial, Helvetica,sans-serif;'>
		      	Un nouveau document de culture a été uploadé sur le backoffice :<br />
		...
		mail($destinatairesTest,"[Souviens-toi Pan !] Document de culture ajouté à l'intranet", $message, $headers);
	}
?>