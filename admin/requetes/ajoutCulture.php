<?php
	...
	if($isLien == 0){
		...
		$target_path = $_SERVER['DOCUMENT_ROOT'] . "/admin/MEDIA/culture/";;
	...
	if(isset($_POST['nomFichier'])) {
		...
		if($isLien == 0){
			$lien = 'http://www.compotedeprod.com/admin/MEDIA/culture/' . $nomFichierLien;
		...
		$message = "<html>
		<head>
		  <title>[CDP] Nouveau document de culture uploadé sur l'intranet</title>
		</head>
		<body>
		  <table>
		  	<tr><td></td></tr>
		...
		mail($destinatairesTest,"[CDP] Document de culture ajouté à l'intranet", $message, $headers);
	}
?>