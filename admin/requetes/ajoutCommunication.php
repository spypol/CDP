<?php
	...
	if(isset($_POST['nomFichier'])) {
		...
		if($isLien == 0){
			...
			$lien = $nomFichierLien;
			$sql = 'INSERT INTO T_COMMUNICATION (communication_NOM, communication_LIEN, communication_VALIDE, communication_CAT) VALUES ("'.$nom.'", "'.$lien.'", 1, 1)';
		}
		...
		$message = "<html>
		<head>
		  <title>[Souviens-toi Pan !] Nouveau document de communication uploadé sur l'intranet</title>
		</head>
		<body>
		  <table>
		  	<tr><td></td></tr>
		    <tr>
		      <td style='font-size:12px;font-weight:bold;font-family: Arial, Helvetica,sans-serif;'>
		      	Un nouveau document de communication a été uploadé sur le backoffice :<br />
		...
		mail($destinataires,"[Souviens-toi Pan !] Document de communication ajouté à l'intranet", $message, $headers);
	}
?>