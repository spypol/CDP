<?php
	...
	if($isLien == 0){
		...
		$target_path = $_SERVER['DOCUMENT_ROOT'] . "/admin/MEDIA/gestion/";;
	...
	if(isset($_POST['nomFichier'])) {
		...
		if($isLien == 0){
			$lien = 'http://www.compotedeprod.com/admin/MEDIA/gestion/' . $nomFichierLien;
		...
		$message = "<html>
		<head>
		  <title>[CDP] Nouveau document de gestion uploadé sur l'intranet</title>
		</head>
		<body>
		  <table>
		  	<tr><td></td></tr>
		...
		mail($destinataires,"[CDP] Document de gestion ajouté à l'intranet", $message, $headers);
	}
?>